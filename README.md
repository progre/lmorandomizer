...
Note: If you want a blind playthrough, do not open `spoilerlog.txt`.

### Launcher behavior

The launcher hooks a DLL when the game starts, applying small modifications to the game via the hook DLL.

- Switches the game script file to load to the one generated by the randomizer
- Redirects the save data location to a folder dedicated to the randomizer
- Skips the window display shown when discovering items with the Handy Scanner
  - Disables the processing of object number 37 (Memo) that is used to invoke the replacement item for object number 32 (ROM), and instead updates only the flag

If you do not want to use the launcher, click **OPEN FOLDER** and manually copy `script.dat` from the folder to `data/script.dat`.
