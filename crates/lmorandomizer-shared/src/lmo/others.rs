use std::{collections::BTreeMap, fmt, num::NonZero, sync::LazyLock};
...
impl MainWeapon {
    ...
    pub fn related_flag(&self) -> Option<NonZero<u16>> {
        match self {
            MainWeapon::Whip => None,
            MainWeapon::ChainWhip => Some(684.try_into().unwrap()),
            MainWeapon::Mace => Some(685.try_into().unwrap()),
            MainWeapon::Knife => Some(686.try_into().unwrap()),
            MainWeapon::KeySword => Some(687.try_into().unwrap()),
            MainWeapon::Axe => Some(688.try_into().unwrap()),
            MainWeapon::Katana => Some(689.try_into().unwrap()),
        }
    }
}
...
impl SubWeapon {
    ...
    pub fn related_flag(&self) -> Option<NonZero<u16>> {
        match self {
            SubWeapon::Shuriken => Some(690.try_into().unwrap()),
            SubWeapon::Touken => Some(691.try_into().unwrap()),
            SubWeapon::Spear => Some(692.try_into().unwrap()),
            SubWeapon::FlareGun => Some(693.try_into().unwrap()),
            SubWeapon::Bomb => Some(694.try_into().unwrap()),
            SubWeapon::Pistol => Some(695.try_into().unwrap()),
            SubWeapon::Weights => None,
            SubWeapon::AnkhJewel => None,
            SubWeapon::Buckler => Some(697.try_into().unwrap()),
            SubWeapon::HandScanner => Some(696.try_into().unwrap()),
