...
use crate::{
    dataset::{
        create_source::create_source, supplements::SupplementFiles, NIGHT_SURFACE_CHEST_COUNT,
        NIGHT_SURFACE_SEAL_COUNT, NIGHT_SURFACE_SUB_WEAPON_COUNT,
...
pub fn randomize(
    ...
    if cfg!(debug_assertions) {
        let main_weapon_count = source.main_weapon_shutters().len();
        debug_assert_eq!(main_weapon_count, script.main_weapons().unwrap().len());
        let sub_weapon_count = source.sub_weapon_shutters().len() + NIGHT_SURFACE_SUB_WEAPON_COUNT;
        ...
        let seal_count = source.seal_chests().len()
    ...
    trace!("create_source {:?}", start.elapsed());
