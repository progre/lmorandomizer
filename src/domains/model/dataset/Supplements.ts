...
export default class Supplements {
  ...
  constructor(
    ...
    const { mainWeapons, subWeapons } = <any>(
      yaml.safeLoad(supplementFiles.weaponsYml)
    );
    const chests = <any>yaml.safeLoad(supplementFiles.chestsYml);
    const seals = <any>yaml.safeLoad(supplementFiles.sealsYml);
    const shops = <any>yaml.safeLoad(supplementFiles.shopsYml);
    const events = parseRequirementsOfEvents(<any>yaml.safeLoad(supplementFiles.eventsYml));
